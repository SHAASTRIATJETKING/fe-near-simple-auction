...
const renderMint = async() => {
    ...
    document.querySelector('#mint-form').addEventListener('submit', async(e) => {
        ...
        try {
            ...
            }, 300000000000000, utils.format.parseNearAmount("0.1"))
            console.log(callRes)
...
const renderCreateAuction = async() => {
    ...
    `
    document.querySelector('#create-auction-form').addEventListener('submit', async(e) => {
        e.preventDefault()
        let tokenId = document.querySelector('#token-id').value
        let startPrice = utils.format.parseNearAmount(document.querySelector('#start-price').value)
        let startTime = Date.parse(document.querySelector('#start-time').value) / 1000
        let endTime = Date.parse(document.querySelector('#end-time').value) / 1000
...
const renderAuction = async() => {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            card.innerHTML = `
                    <div class="card-header">
                        ...
                        <p class="token-id">Token ID: ${auction.auction_token}</p>
                        <h3 class="card-title">Title: ${token.metadata.title}</h3>
                        <p class="owner"> Owner ID: ${auction.owner}</p>
                        <p class="current-price">Current Price: ${auction.current_price / 10 ** 24} NEAR</p>
                        <p class="current-winner">Current winner: ${auction.winner || 'no winner'}</p>
                        <p class="end-time">End: ${new Date(auction.end_time / 1_000_000)}</p>
                    </div>
                    <div class="card-content">
                        <div class="card-img">
                            <img  src="${data.image}" />
    ...
    document.querySelector('#bid').onclick = async() => {
        ...
        let bidPrice = utils.format.parseNearAmount(document.querySelector('#bid-price').value)
        console.log(auctionId, bidPrice)
        let res = await contract.bid({ auction_id: auctionId }, 300000000000000, bidPrice)
        console.log(res)
    }
